...
// с заданными числами вычисляем функцию Аккермана (m,n)  
// Красивая печать.
...
int EnterNumb(string Name ) //ввод корректности целого числа
{
 ...
 int Akkerman(int m, int n) //Метод рекурсии функции Аккермана (m,n)
{
    if (m==0)
        return n+1;
    if (m!=0 && n==0)
...
Console.Clear();
int M1=EnterNumb("m "); // задать число m
int N1=EnterNumb("n "); // задать число n
//nt M1=3;
//int N1=2;
